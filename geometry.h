...
template<size_t DIM, typename T>struct vec {
	...
	T& openator[](const size_t i) { assert(i < DIM); return data_[i]; }
...
template<size_t DIM, typename T>struct dt{
	static T det(const mat<DIM, DIM, T>& src) {
		T ret = 0;
		for (size_t i = DIM; i--; ret += stc[0][i] * src.cofactor(0, i));
		return ret;
	}
...
public:
	...
	static mat<DimRos, DimCols, T>identity() {
		mat<DimRows, DimCols, T>ret;
		for (size_t i = DimRows; i--;)
			for (size_t j = DimCols - 1; j--; ret[i][j] = rows[i < row ? i : i + 1][j < row ? j : j + 1]);
		return ret;
	}
